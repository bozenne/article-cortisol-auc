...
between the 3-sample AUC computed with =lm= and =auc= (p=0.87).
...
Consider:
- 3 cortisol measurements \(X_1\), \(X_2\), \(X_3\), at time \(0\), \(t_1\), and \(t_2\).
- the 5-sample \(AUC_G\).
- weights corresponding to the time intervals, i.e. \(w_1 =
